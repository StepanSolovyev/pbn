...
namespace RusBounds
{
    ...
    public struct emitent
    {
        ...
        /// </summary> 
        public emitent(int emitid, uint bondsCounter=0)
        {
            emit            =  emitid;
            INN             = 0;
            Bonds = new bond[bondsCounter];
    ...
    public struct bond
    {
        public string NameOfPeriod; //наименование выпуска  ( ссылка)
        public string NumberOfRegistration; //номер регистрации
        public string StatusOfPeriod; //состояние выпуска
        public string StartData; //начало размещения
        public string EndData; //дата погашения
        ...
        /// <summary>  
        ///  Конструктор облигиций заполняет все поля пустыми значениями.
        /// </summary> 
        public bond(uint bond_id)
        {
            bondid = bond_id;
            NameOfPeriod = "";
        ...
        emitent GetEmitentData(string emithref)
        {
            ...
            try
            {
                ...
                foreach(HtmlNode node in EmitentInfo)
                {
                    ...
                    switch (swch)
                    {
                        case "Наименование выпуска":
                            Emit.NameOfPeriod = node.InnerText.Split(':')[1];
                            break;
                        case "Номер регистрации":
                            Emit.NumberOfRegistration = node.InnerText.Split(':')[1];
                            break;
                        case "Состояние выпуска":
                            Emit.StatusOfPeriod = node.InnerText.Split(':')[1];
                            break;

                        case "Начало размещения":
                            Emit.StartData = node.InnerText.Split(':')[1];
                            break;
                        ...
                        case "Дата погашения":
                            Emit.EndData = node.InnerText.Split(':')[1];
                            break;

                        case "ISIN код":
                            Emit.ISIN = node.InnerText.Split(':')[1];
                            break;
